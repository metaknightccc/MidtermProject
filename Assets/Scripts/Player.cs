...
public class Player : MonoBehaviour
{
        ...
        if (endLag <= 0) {
            ...
            if(controls.Player.NormalAttack.triggered) {
                if (grounded) { // Grounded attacks
                    ...
                    } else if (move.y < -.5) { // Down tilt
                        endLag = 0.25f;
                        Debug.Log("Down tilt");
                        StartCoroutine(attackHitbox(0.117f, 1));
                    } else if (move.x > .2 || move.x < -.2) { // Side tilt
                        endLag = 0.65f;
                        Debug.Log("Side tilt");
                    ...
                    } else if (move.y < -.5) { // Down Air
                        Debug.Log("Down Air");
                    } else if (move.x * direction > 0.5) { // Forward Air
                        Debug.Log("Forward Air");
                        endLag = 0.5f;
                        StartCoroutine(attackHitbox(0.133f, 5));
                    } else if (move.x * direction < -0.5) { // Back Air
                        Debug.Log("Back Air");
                        endLag = 0.75f;
    ...
    {
        // Visualize Hitboxes
        for (int i = 4; i < hitboxes.Length; i++) {
            Gizmos.DrawWireSphere(hitboxes[i].position, hitboxSizes[i]);
        }
        // Alternatively picking a single Hitbox
        // Gizmos.DrawWireSphere(hitboxes[3].position, hitboxSizes[3]);
    }
}
